...
%
% For example, if ECR activates more during the AD recording than its own,
% that value is used. This gives the most representative MVC possible.
%
% Workflow:
...
smooth_s  = 0.5;  % smoothing window for peak detection [s]
...
for fi = 1:n_files
    ...
    % Peak of the smoothed signal per channel
    for ch = 1:n_muscles
        rms_smooth          = movmean(rms_all(:, ch), smooth_samp);
        peak_matrix(ch, fi) = max(rms_smooth);
    end
